...
interface MegaCategoryMaidModalProps {
  open: boolean;
  onClose: () => void;
  megaJobId: string;
...
export function MegaCategoryMaidModal({
  ...
  useEffect(() => {
    ...
    const resolveCountry = async () => {
      ...
      try {
        const res = await fetch(`/api/mega-jobs/${megaJobId}`, { credentials: 'include' });
        if (res.ok) {
  ...
  const handleCompute = async () => {
    ...
    try {
      ...
      while (data.phase !== 'done' && data.phase !== 'error') {
        ...
        data = await safePollFetch(`/api/mega-jobs/${megaJobId}/export/category-poll`, {
          method: 'POST',
          credentials: 'include',
        });
      }
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        ...
        {!country && !loadingCountry && (
          <div className="py-8 text-center text-muted-foreground">
            ...
            <p className="text-sm mt-1">Set the country for this mega-job first.</p>
          </div>
        )}
              ...
              <div className="border rounded-lg p-4 space-y-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    ...
                    {minDwell > 0 ? ` (≥${minDwell} min dwell)` : ''} across all sub-jobs
                  </p>
