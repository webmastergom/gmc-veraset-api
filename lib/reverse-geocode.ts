...
// ── Pre-computed geocode cache ─────────────────────────────────────────
// For large countries (MX 40MB, US 92MB), loading GeoJSON + building spatial
// index exceeds Vercel's 60s limit. Pre-computed CSV caches (~400KB) stored
// in S3 at config/geocode-cache/{CC}.csv provide O(1) lookup by grid key.
// Generated by: npx tsx scripts/generate-geocode-cache.ts

const geocodeCsvCache = new Map<string, Map<string, string>>(); // country → (latKey,lngKey → zipcode)
const GEOCODE_CACHE_PRECISION = 10; // 1 decimal: latKey = round(lat,1) * 10

/**
 * Load pre-computed geocode cache from S3 for a country.
 * Returns true if cache was loaded, false if not available.
 * Cache is stored at config/geocode-cache/{CC}.csv with format: latKey,lngKey,zipcode
 */
export async function loadGeocodeCache(countryCode: string): Promise<boolean> {
  if (!countryCode) return false;
  const cc = countryCode.toUpperCase();
  if (geocodeCsvCache.has(cc)) return true;
  ...
  try {
    ...
    const res = await s3.send(new GetObjectCommand({
      Bucket: S3_BUCKET,
      Key: `config/geocode-cache/${cc}.csv`,
    }));
    const csv = await res.Body?.transformToString() || '';
    ...
    geocodeCsvCache.set(cc, map);
    console.log(`[GEOCODE] Loaded pre-computed cache for ${cc}: ${map.size} entries`);
    return true;
  } catch {
    return false;
  }
...
/**
 * Look up a zipcode from the pre-computed cache.
 * Returns postcode or null if not in cache.
 */
function getZipcodeFromCache(lat: number, lng: number): { country: string; postcode: string } | null {
  // Check all loaded country caches
  for (const [cc, map] of geocodeCsvCache) {
    const latKey = Math.round(lat * GEOCODE_CACHE_PRECISION);
    const lngKey = Math.round(lng * GEOCODE_CACHE_PRECISION);
    const zc = map.get(`${latKey},${lngKey}`);
    if (zc) return { country: cc, postcode: zc };
  }
  return null;
}
...
/**
 * Batch reverse geocode. Tries pre-computed cache first (fast), falls back
 * to GeoJSON spatial index (slower but precise).
...
export async function batchReverseGeocode(
  ...
  // Fall back to GeoJSON if no cache available
  if (!hasCache) {
  ...
  for (const p of points) {
    // Try cache first (fast path for large countries)
    if (hasCache) {
      const cached = getZipcodeFromCache(p.lat, p.lng);
      if (cached) {
        results.push({
          type: 'geojson_local',
          country: cached.country,
    ...
    // Full GeoJSON lookup (precise, includes city/province/region)
